<template>
  <div class="two-twoCheckupelse-dialog-container">
    <el-dialog :title="state.dialog.title" v-model="state.dialog.isShowDialog" width="60%">
          <el-form ref="twoCheckupelseDialogFormRef" :model="state.ruleForm" :rules="state.rules" size="default" label-width="90px">
            <el-row :gutter="35">
...
<script setup lang="ts" name="dictDialog">
  ...
  const onSubmit = () => {
    // 验证表单
    Promise.all([
      currentValidate(twoCheckupelseDialogFormRef),
    ]).then(res => {
